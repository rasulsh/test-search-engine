...
final class InstallPage
{
    ...
    private const FIELDS = [
        ...
        'مدل' => [
            ...
            'normalization_version' => [
                'نسخهٔ نرمال‌سازی',
                'پیش‌فرض، نسخهٔ خودِ کد است و با هر به‌روزرسانی خودکار همراه می‌شود؛ آن را تغییر ندهید.',
            ],
        ],
        'فروشگاه' => [
            'store_base' => [
        ...
        'جستجوی معنایی (سرور VPS)' => [
            'vps_url' => [
                'نشانی سرویس VPS',
                'مثل https://vps.example.com:8600 ؛ اگر خالی بماند، فقط جستجوی کلیدواژه‌ای انجام می‌شود.',
            ],
            'vps_token' => ['توکن سرویس VPS', 'همان VPS_TOKEN روی سرور VPS (دست‌کم ۱۶ نویسه، بدون فاصله).'],
            'vps_timeout_ms' => [
                'مهلت پاسخ VPS (میلی‌ثانیه)',
                'اگر VPS در این مدت پاسخ ندهد، نتایج فقط کلیدواژه‌ای برمی‌گردند.',
            ],
        ],
    ...
    private const INVALID = [
        ...
        'vps_url' => 'نشانی کامل با http:// یا https://',
        'vps_token' => 'دست‌کم ۱۶ نویسهٔ انگلیسی، بدون فاصله؛ با نشانی VPS الزامی است.',
    ];
